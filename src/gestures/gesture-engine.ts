...
export class GestureEngine {
    ...
    constructor(renderer: THREE.WebGLRenderer, camera: THREE.Camera, cardManager: CardManager) {
    ...
    update(frame: XRFrame) {
        ...
        for (const inputSource of inputSources) {
            ...
            if (inputSource.targetRaySpace) {
                const targetRayPose = frame.getPose(inputSource.targetRaySpace, this.renderer.xr.getReferenceSpace()!);
                if (targetRayPose) {
                    // Raycast logic
                    const origin = new THREE.Vector3(targetRayPose.transform.position.x, targetRayPose.transform.position.y, targetRayPose.transform.position.z);
                    const direction = new THREE.Vector3(0, 0, -1).applyQuaternion(new THREE.Quaternion(targetRayPose.transform.orientation.x, targetRayPose.transform.orientation.y, targetRayPose.transform.orientation.z, targetRayPose.transform.orientation.w));
