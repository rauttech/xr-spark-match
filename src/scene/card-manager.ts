...
export class CardManager {
    ...
    constructor(scene: THREE.Scene, matchEngine: MatchEngine, sparkManager: SparkManager) {
        this.scene = scene;
    ...
    initializeCards(profiles: Profile[]) {
        ...
        profiles.forEach((profile, index) => {
            ...
            card.position.set(Math.cos(angle) * radius, 1.6, Math.sin(angle) * radius);
    ...
    update(time: number, camera: THREE.Camera) {
        ...
        this.cards.forEach((card, id) => {
            card.position.y = 1.6 + Math.sin(time * 0.001 + parseFloat(id.charCodeAt(0).toString())) * 0.1;
        });
